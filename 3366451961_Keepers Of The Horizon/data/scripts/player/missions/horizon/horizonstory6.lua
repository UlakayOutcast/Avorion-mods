...
mission.data.description = {
    { text = "Вы отогнали основную часть флота Хранителей Горизонта, теперь пришло время выследить их подбитые линкоры. Покончите с ними, чтобы расчистить себе путь." },
    ...
    { text = "Уничтожьте линкоры Горизонта", bulletPoint = true, fulfilled = false, visible = false },
    { text = "(Рекомендуется) Уничтожьте боевую линию Горизонта", bulletPoint = true, fulfilled = false, visible = false },
    { text = "Уничтожьте оставшиеся корабли Горизонта", bulletPoint = true, fulfilled = false, visible = false },
    { text = "Уничтожьте прототипы оружия Горизонта", bulletPoint = true, fulfilled = false, visible = false },
    { text = "(Рекомендуется) Уничтожьте Альфа-оружие, чтобы устранить точечную оборону", bulletPoint = true, fulfilled = false, visible = false },
    { text = "(Рекомендуется) Используйте боеприпасы, пробивающие щиты, чтобы уничтожить Бета-оружие", bulletPoint = true, fulfilled = false, visible = false }
}
...
mission.phases[1].onBeginServer = function()
    ...
    local _Mail = Mail()
	_Mail.text = Format("Привет, приятель.\n\nЯ отследил линкоры до (%1%:%2%). Встреться со мной там. Пришло время навсегда покончить с боевым флотом Горизонта.\n\nВарланс", _X, _Y)
	_Mail.header = "Нашел линкоры"
    ...
    callback = function()
        ...
        if atTargetLocation() then
            ...
            if cruiserCt == 0 and not mission.data.custom.stage2OrdersSet and not mission.data.custom.playerRushedBattleships then
                ...
                HorizonUtil.varlanceChatter("Их оборонительный экран исчез. Теперь линкорам придется вступить в бой с нами напрямую.")
...
function kothStory6_onPhase2Dialog(battleshipID)
    ...
    d0.text = "Вот они, дружок. Наши загнанные в угол крысы."
    d0.answers = {
        { answer = "Это чертовски большие крысы.", followUp = d1 }
    }

    d1.text = "Пусть их размер тебя не пугает - мы уже однажды разорвали их на куски. Наши когти острее."
    d1.followUp = d2

    d2.text = "Сначала уничтожь линию крейсеров. Пока у линкоров есть оборонительный экран, они могут держаться на расстоянии и обстреливать нас торпедами и пушечным огнем."
    d2.followUp = d3

    d3.text = "Давай двигаться. Мы покажем им, как высоко мы можем взлететь."
...
function kothStory6_onPhase4Dialog(weaponID)
    ...
    d0.text = "Мы опоздали... они все мертвы."
    d0.followUp = d1

    d1.text = "Вы заплатите за то, что сделали, мясники."
    d1.followUp = d2

    d2.text = "Мясники? Это вы поставляете высокотехнологичное оружие пиратам."
    d2.followUp = d3

    d3.text = "Вы не понимаете. Чтобы остаться такими, какие мы есть... это лишь вопрос времени, когда галактика снова будет разорвана на части."
    d3.followUp = d4

    d4.text = "Их бесконечно много. Их не остановить. Мы должны развиваться, и Ксотан - следующий шаг... за горизонт!"
    d4.followUp = d5

    d5.text = "Понятно. Вы явно потеряли связь с реальностью."
    d5.followUp = d6

    d6.text = "Будь начеку, дружок. Показания, которые мы получаем с этих кораблей, вызывают беспокойство."
    d6.followUp = d7

    d7.text = "Предварительное сканирование показывает, что у альфа-юнита есть какая-то продвинутая система точечной защиты. У бета-юнита мощная система экранирования."
    d7.followUp = d8

    d8.text = "Похоже, она не закалена. Подкалиберные торпеды или импульсные пушки разорвут ее на части."
    d8.followUp = d9

    d9.text = "Есть один трюк, которому я научился у Кавалеров, и он здесь сработает. Давай закончим с этим."
...
function kothStory6_onPhase5Dialog(varlanceID)
    ...
    d0.text = "Это был кошмар."
    d0.followUp = d1

    d1.text = "Я думаю, мы только что избавили галактику от столкновения с чем-то действительно ужасающим."
    d1.followUp = d2
    
    d2.text = "Эти корабли - эти капитаны - были на голову выше идиота, командующего этим украденным линкором. Я посмотрю, какую информацию мы сможем выудить из компьютерных систем этих обломков."
    d2.followUp = d3

    d3.text = "Тем временем наш путь ясен. Следующей целью будет их верфь."
    d3.followUp = d4

    d4.text = "Я свяжусь с тобой, когда придет время, дружок."
