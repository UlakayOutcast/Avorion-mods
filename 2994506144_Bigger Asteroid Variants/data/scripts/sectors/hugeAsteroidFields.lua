...
local SectorGenerationChance = 75;
-- Probability for asteroids to have resources
local ResourceProbability = 0.15;
-- Chance for the sector to be resource rich
local ResourceRichChance = 0.45;
...
local NumAsteroidFields = getFloat(2,3);
local NumBigAsteroidFields = getFloat(0,2);
local NumNormalAsteroidFields = getFloat(3,5);
...
function SectorTemplate.generate(player, seed, x, y)
    ...
    if isResourceRich then
        print("Generating huge resource rich asteroid sector!");
    else
        print("Generating huge asteroid sector!")
    end

    local generator = SectorGenerator(x, y)

    for i = 1, NumAsteroidFields do
        local position = generator:createHugeAsteroidField(resProbability)
        if math.random() < multiProbability then generator:createHugeMultiAsteroid(position) end
    end

    for i = 1, NumBigAsteroidFields do
