...
function updateServer(tick)
    --Updates turrets every ~15s
    if ticks % 750 == 0 then updateTurrets() end
    ticks = ticks + 1
            ...
            if tAI.targetedEntity ~= nil then
				if not turret.weaponsPlayerControlled then
					...
					if valid(target) and target.isShip then
						...
						local targetRadius = lowestValue(target.size)
						if length(relativePos) < targetRadius * 3 then
							targetRadius = 0
						end
...
function addRandomSpray(vector, radius)
	return vec3(vector.x + (math.random() - 0.5) * radius, vector.y + (math.random() - 0.5) * radius, vector.z + (math.random() - 0.5) * radius)
end