...
local function fillWeaponTooltipData(obj, tooltip, wpn, typ)
    ...
    if lyr.damagePerProjectile > 0 and not isCivilTurret(wpn) then 
		...
		if wpn.hullDamageMultiplicator == wpn.shieldDamageMultiplicator then
			...
			if typ == "turret" and obj.slots > 0 then
				line.ltext = "Средний "..(lyr.usesEDPS and "eDPS" or "DPS").." /слот" --lyr_nt
				line.rtext = round((aDPS) / obj.slots, 1)
			elseif showFighterDpsPerSize and typ == "fighter" and obj.volume > 0 then
				line.ltext = "Средний "..(lyr.usesEDPS and "eDPS" or "DPS").." /размер" --lyr_nt
				line.rtext = round((aDPS) / obj.volume, 1)
			else
				line.ltext = "Средний "..(lyr.usesEDPS and "eDPS" or "DPS") --lyr_nt
			...
			local line = TooltipLine(lineHeight, fontSize)
			if typ == "turret" and obj.slots > 0 then
				line.ltext = "Корпус "..(lyr.usesEDPS and "eDPS" or "DPS").." /слот" --lyr_nt
				line.rtext = round((hDPS) / obj.slots, 1)
			elseif showFighterDpsPerSize and typ == "fighter" and obj.volume > 0 then
				line.ltext = "Корпус "..(lyr.usesEDPS and "eDPS" or "DPS").." /размер" --lyr_nt
				line.rtext = round((hDPS) / obj.volume, 1)
			else
				line.ltext = "Корпус "..(lyr.usesEDPS and "eDPS" or "DPS") --lyr_nt
			...
			local line = TooltipLine(lineHeight, fontSize)
			if typ == "turret" and obj.slots > 0 then
				line.ltext = "Щит "..(lyr.usesEDPS and "eDPS" or "DPS").." /слот" --lyr_nt
				line.rtext = round((sDPS) / obj.slots, 1)
			elseif showFighterDpsPerSize and typ == "fighter" and obj.volume > 0 then
				line.ltext = "Щит "..(lyr.usesEDPS and "eDPS" or "DPS").." /размер" --lyr_nt
				line.rtext = round((sDPS) / obj.volume, 1)
			else
				line.ltext = "Щит "..(lyr.usesEDPS and "eDPS" or "DPS") --lyr_nt
			...
			local line = TooltipLine(lineHeight, fontSize)
			if wpn.continuousBeam then
				line.ltext = "Урон корпусу за тик" --lyr_nt
				line.rtext = round(lyr.damagePerShot * wpn.hullDamageMultiplicator, 1)
			elseif lyr.isBurstFire then
				line.ltext = "Урон корпусу от взрыва" --lyr_nt
				line.rtext = lyr.projectilesPerCycle.."x"..round(lyr.damagePerProjectile * wpn.hullDamageMultiplicator, 1)
			else
				line.ltext = "Урон корпусу" --lyr_nt
				line.rtext = lyr.isMultiShot and lyr.projectilesPerTurret.."x"..round(lyr.damagePerProjectile * wpn.hullDamageMultiplicator, 1) or round(lyr.damagePerProjectile * wpn.hullDamageMultiplicator, 1)
			end
			line.icon = "data/textures/icons/health-normal.png";
			addLine(tooltip,line)
    ...
    elseif isCivilTurret(wpn) then 
        ...
        if tType.type == "Mining" then
			...
			local line = TooltipLine(lineHeight, fontSize)
			line.ltext = "Эффективность добычи" --lyr_nt
        ...
        if tType.type == "Salvaging" then
			...
			local dps
			if typ == "turret" and obj.slots > 0 then
				line.ltext = "Утилизация DPS /слот" --lyr_nt
				dps = round((lyr.damagePerSecond * wpn.hullDamageMultiplicator) / obj.slots, 1)
			elseif showFighterDpsPerSize and typ == "fighter" and obj.volume > 0 then
				line.ltext = "Утилизация DPS /размер" --lyr_nt
				dps = round((lyr.damagePerSecond * wpn.hullDamageMultiplicator) / obj.volume, 1)
			else
				line.ltext = "Утилизация DPS" --lyr_nt
				dps = round(lyr.damagePerSecond * wpn.hullDamageMultiplicator, 1)
			end
			line.rtext = dps
			...
			local line = TooltipLine(lineHeight, fontSize)
			line.ltext = "Эффективность Утилизации" --lyr_nt
			line.rtext = round(wpn[tType.access] * 100, 1).."%"
			line.icon = "data/textures/icons/recycle.png";
			addLine(tooltip,line)
		end
	elseif wpn.otherForce ~= 0 or wpn.selfForce ~= 0 or wpn.holdingForce ~= 0 then
		-- force turrets
		...
		if wpn.holdingForce > 0 then
			local line = TooltipLine(lineHeight, fontSize)
			line.ltext = "Force Power"%_t
			line.rtext = toReadableValue(wpn.holdingForce, "N")
			line.icon = "data/textures/icons/back-forth.png";
			addLine(tooltip,line)
		end
	elseif obj.hullRepairRate > 0 or obj.shieldRepairRate > 0 then
		if obj.hullRepairRate > 0 then
			...
			local line = TooltipLine(lineHeight, fontSize)
			if typ == "turret" and obj.slots > 0 then
				line.ltext = "Корпус HPS /слот" --lyr_nt
				line.rtext = round(obj.hullRepairRate / obj.slots, 1) 
			elseif showFighterDpsPerSize and typ == "fighter" and obj.volume > 0 then
				line.ltext = "Корпус HPS /размер" --lyr_nt
				line.rtext = round(obj.hullRepairRate / obj.volume, 1) 
			else
				line.ltext = "Корпус HPS" --lyr_nt
	...
	if wpn.continuousBeam then
		-- beam gun attribute
		local line = TooltipLine(lineHeight, fontSize)
		line.ltext = "Скорость тика" --lyr_nt
	...
	elseif lyr.isBurstFire then
		...
		local line = TooltipLine(lineHeight, fontSize)
		line.ltext = "Скорость огня" --lyr_nt
		line.rtext = round(lyr.shotsPerSecond, 2)
		line.icon = "data/textures/icons/bullets.png";
		addLine(tooltip,line)
	end
...
--
	...
	if wpn.blockPenetration > 1 then
		local line = TooltipLine(lineHeight, fontSize)
		line.ltext = "Пробивание корпуса" --lyr_nt
	...
	if wpn.shieldPenetration > 0 then
		local line = TooltipLine(lineHeight, fontSize)
		line.ltext = "Пробивание щита" --lyr_nt
	...
	local line = TooltipLine(lineHeight, fontSize)
	line.ltext = "Точность"%_t
	line.rtext = (wpn.continuousBeam or wpn.accuracy == 1) and "Абсолют" or round(wpn.accuracy * 100, 1).."%"
	line.icon = "data/textures/icons/gunner.png";
	addLine(tooltip,line)

	local line = TooltipLine(lineHeight, fontSize)
	line.ltext = "Скорость" --lyr_nt
	line.rtext = wpn.isBeam and "Мгновенный" or round(wpn.pvelocity*10, 0).."m/s"
	line.icon = "data/textures/icons/speedometer.png";
	addLine(tooltip,line)

	local line = TooltipLine(lineHeight, fontSize)
	line.ltext = "Диапазон"%_t
	...
	if typ == "turret" then
		local line = TooltipLine(lineHeight, fontSize)
		line.ltext = "Скорость отслеживания" --lyr_nt
	...
	if lyr.demandsPower then
		local line = TooltipLine(lineHeight, fontSize)
		line.ltext = "Требуется питание" --lyr_nt
		...
		local line = TooltipLine(lineHeight, fontSize)
		line.ltext = "Базовая потребность" --lyr_nt
		...
		local line = TooltipLine(lineHeight, fontSize)
		line.ltext = "Нормализация" --lyr_nt
	...
	elseif lyr.drainsEnergy then
		local line = TooltipLine(lineHeight, fontSize)
		line.ltext = "Потребляет энергию" --lyr_nt
		line.lcolor = fadedCenterTextColor
		line.icon = "data/textures/icons/info.png";
		line.iconColor = fadedIconColor
		tooltip:addLine(line)
		...
		local line = TooltipLine(lineHeight, fontSize)
		line.ltext = "Базовое истощение" --lyr_nt
	...
	elseif lyr.generatesHeat then
		local line = TooltipLine(lineHeight, fontSize)
		line.ltext = "Генерирует тепло" --lyr_nt
		line.lcolor = fadedCenterTextColor
		line.icon = "data/textures/icons/info.png";
		line.iconColor = fadedIconColor
		tooltip:addLine(line)
		...
		local line = TooltipLine(lineHeight, fontSize)
		line.ltext = "Непрерывная стрельба"
...
local function fillDescriptions(obj, wpn, tooltip, isFighter)
	...
	-- REMOVED IN 2.0
	--if not isFighter and obj.automatic ~= nil then
	if not isFighter and obj.slotType == TurretSlotType.PointDefense then
		special[#special+1] = {
			ltext = "Автонавидение", --lyr_nt
	...
	if obj.simultaneousShooting and obj.numWeapons > 1 then
		special[#special+1] = {
			ltext = "Синхронизированное оружие", --lyr_nt
	...
	if obj.shotsPerFiring > 1 then
		special[#special+1] = {
			ltext = "Множественные снаряды", --lyr_nt
	...
	if obj.coolingType == CoolingType.BatteryCharge then
        special[#special+1] = {
			ltext = "Заряд аккумулятора", --lyr_nt
	...
	if obj.seeker then
		special[#special+1] = {
			ltext = "Управляемые ракеты", --lyr_nt
	...
	if descriptions["Ionized Projectiles"] then
		special[#special+1] = {
			ltext = "Ионизированные снаряды", --lyr_nt
	...
	if wpn then
    	...
    	if wpn.metalRawEfficiency > 0 then
    	    ...
    	    line.ltext = "Разбивает сплавы на металлолом"
    	    line.litalic = true
    	...
    	if wpn.stoneRawEfficiency > 0 then
    	    local line = TooltipLine(descriptionLineHeight, descriptionFontSize)
    	    line.ltext = "Разбивает камень на руды"
    	    line.litalic = true
    	    table.insert(additional, line)
			extraLines = extraLines + 1
    	end
    	if wpn.stoneRefinedEfficiency > 0 then
    	    local line = TooltipLine(descriptionLineHeight, descriptionFontSize)
    	    line.ltext = "Уточнение: Перерабатывает камень в ресурсы"
    	    line.litalic = true
    	    table.insert(additional, line)
			extraLines = extraLines + 1
    	end
    	if wpn.metalRefinedEfficiency > 0 then
    	    local line = TooltipLine(descriptionLineHeight, descriptionFontSize)
    	    line.ltext = "Уточнение: Перерабатывает сплавы в ресурсы"
    	    line.litalic = true
    	    table.insert(additional, line)
			extraLines = extraLines + 1
		end
		...
		if wpn.shieldDamageMultiplicator > 1 then
    	    local line = TooltipLine(descriptionLineHeight, descriptionFontSize)
    	    line.ltext = "${bonus} Урон, наносимый щитам"%_t % {bonus = string.format("%+i%%", (wpn.shieldDamageMultiplicator - 1) * 100)}
    	    table.insert(additional, line)
			extraLines = extraLines + 1
		end
		if wpn.hullDamageMultiplicator > 1 then
    	    local line = TooltipLine(descriptionLineHeight, descriptionFontSize)
    	    line.ltext = "${bonus} Повреждение корпуса"%_t % {bonus = string.format("%+i%%", (wpn.hullDamageMultiplicator - 1) * 100)}
			table.insert(additional, line)
			extraLines = extraLines + 1
		end

        if obj.damageType == DamageType.Fragments then
			local line = TooltipLine(descriptionLineHeight, descriptionFontSize)
            line.ltext = "Больше повреждений истребителям, торпедам"
			table.insert(additional, line)
			extraLines = extraLines + 1
		elseif obj.damageType == DamageType.Electric then
			local line = TooltipLine(descriptionLineHeight, descriptionFontSize)
            line.ltext = "+100% урона техническим блокам"
			table.insert(additional, line)
			extraLines = extraLines + 1
		end
	end
...
local function fillObjectTooltipHeader(obj, tooltip, title, isValidObject, typ, customName)
	...
	if typ == "torpedo" then
		line.ltext = "Технология: "..round(obj.tech, 1)  --lyr_nt
		line.ctext = tostring(obj.rarity)
		--line.rtext = obj.material.name
		line.ccolor = obj.rarity.color
		--line.rcolor = obj.material.color
	else
		line.ltext = "Технология: "..round(obj.averageTech, 1)  --lyr_nt
	...
	if not isValidObject then 
		...
		line.citalic = true
		line.ctext = "этот "..typ.." не имеет оружия"; tooltip:addLine(line)
		line.ctext = typ.."должны иметь хотя бы один"; tooltip:addLine(line)
		line.ctext = "пропуск расчетов мод DTT"; tooltip:addLine(line)
		line.ctext = ""; tooltip:addLine(line)
	end;
end
	...
	if not customName then
        ...
        if turret.stoneRefinedEfficiency > 0 or turret.metalRefinedEfficiency > 0
            ...
            if turret.itemType == InventoryItemType.Turret then
                ...
                if turret.numVisibleWeapons == 1 then
                    title = "${material} ${weaponPrefix} Турель"%_t % tbl
                elseif turret.numVisibleWeapons == 2 then
                    title = "Двойной ${material} ${weaponPrefix} Турель"%_t % tbl
                elseif turret.numVisibleWeapons == 3 then
                    title = "Тройной ${material} ${weaponPrefix} Турель"%_t % tbl
                elseif turret.numVisibleWeapons == 4 then
                    title = "Четверной ${material} ${weaponPrefix} Турель"%_t % tbl
                else
                    title = "Мульти ${material} ${weaponPrefix} Турель"%_t % tbl
                end

            else
                -- turret template
                if turret.numVisibleWeapons == 1 then
                    title = "${material} ${weaponPrefix} План"%_t % tbl
                elseif turret.numVisibleWeapons == 2 then
                    title = "Двойной ${material} ${weaponPrefix} План"%_t % tbl
                elseif turret.numVisibleWeapons == 3 then
                    title = "Тройной ${material} ${weaponPrefix} План"%_t % tbl
                elseif turret.numVisibleWeapons == 4 then
                    title = "Четверной ${material} ${weaponPrefix} План"%_t % tbl
                else
                    title = "Мульти ${material} ${weaponPrefix} План"%_t % tbl
                end
            end
        ...
        elseif turret.coaxial then
            if turret.itemType == InventoryItemType.Turret then
                -- turret
                if turret.numVisibleWeapons == 1 then
                    title = "Коаксиальный ${weaponPrefix}"%_t % tbl
                elseif turret.numVisibleWeapons == 2 then
                    title = "Двойной коаксиальный ${weaponPrefix}"%_t % tbl
                elseif turret.numVisibleWeapons == 3 then
                    title = "Тройной коаксиальный ${weaponPrefix}"%_t % tbl
                elseif turret.numVisibleWeapons == 4 then
                    title = "Четверной коаксиальный ${weaponPrefix}"%_t % tbl
                else
                    title = "Мульти коаксиальный ${weaponPrefix}"%_t % tbl
                end

            else
                -- turret template
                if turret.numVisibleWeapons == 1 then
                    title = "Коаксиальный ${weaponPrefix} План"%_t % tbl
                elseif turret.numVisibleWeapons == 2 then
                    title = "Двойной коаксиальный ${weaponPrefix} План"%_t % tbl
                elseif turret.numVisibleWeapons == 3 then
                    title = "Тройной коаксиальный ${weaponPrefix} План"%_t % tbl
                elseif turret.numVisibleWeapons == 4 then
                    title = "Четверной коаксиальный ${weaponPrefix} План"%_t % tbl
                else
                    title = "Мульти коаксиальный ${weaponPrefix} План"%_t % tbl
                end
            end
            ...
            if turret.itemType == InventoryItemType.Turret then
                -- turret
                if turret.numVisibleWeapons == 1 then
                    title = "${weaponPrefix} Турель"%_t % tbl
                elseif turret.numVisibleWeapons == 2 then
                    title = "Двойной ${weaponPrefix} Турель"%_t % tbl
                elseif turret.numVisibleWeapons == 3 then
                    title = "Тройной ${weaponPrefix} Турель"%_t % tbl
                elseif turret.numVisibleWeapons == 4 then
                    title = "Четверной ${weaponPrefix} Турель"%_t % tbl
                else
                    title = "Мульти ${weaponPrefix} Турель"%_t % tbl
                end

            else
                -- turret template
                if turret.numVisibleWeapons == 1 then
                    title = "${weaponPrefix} План"%_t % tbl
                elseif turret.numVisibleWeapons == 2 then
                    title = "Двойной ${weaponPrefix} План"%_t % tbl
                elseif turret.numVisibleWeapons == 3 then
                    title = "Тройной ${weaponPrefix} План"%_t % tbl
                elseif turret.numVisibleWeapons == 4 then
                    title = "Четверной ${weaponPrefix} План"%_t % tbl
                else
                    title = "Мульти ${weaponPrefix} План"%_t % tbl
                end
            end
        end
    end
...
function makeFighterTooltip(fighter, other)
	...
	-- durability
	if fighter.durability ~= nil then
		local line = TooltipLine(lineHeight, fontSize)
		line.ltext = "Долговечность"%_t
	...
	-- maneuverability
	if fighter.turningSpeed ~= nil then
		local line = TooltipLine(lineHeight, fontSize)
		line.ltext = "Маневренность"%_t
	...
	-- velocity
	if fighter.maxVelocity ~= nil then
		local line = TooltipLine(lineHeight, fontSize)
	...
	-- crew requirements (removed in 2.0)
	local crew = 1
	--if fighter.crew ~= nil then
	--	crew = fighter.crew
	--end
	...
	local num = round(crew)
	local job = pilot:name(crew)
	if num == 1 then
		job = job:sub(1, -2)
	end
	line.rtext = pilot:name(crew)
...
function getCivilTurretType( weapon )
    ...
    t.mode = ((weapon.stoneRawEfficiency > 0) or (weapon.metalRawEfficiency > 0)) and "Сырье" or "Переработка"

    t.access =  (t.mode == "Сырье" and t.type == "Добыча") and "stoneRawEfficiency" or
                (t.mode == "Переработка" and t.type == "Добыча") and "stoneRefinedEfficiency" or
                (t.mode == "Сырье" and t.type == "Утилизация") and "metalRawEfficiency" or
                (t.mode == "Переработка" and t.type == "Утилизация") and "metalRefinedEfficiency";
    return t
end
